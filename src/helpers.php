...
if ( ! function_exists('app_tab')) {
    function app_tab(int $spaces = 4): string
    {
        return str_repeat(' ', $spaces);
    }
}

if ( ! function_exists('app_tabs')) {
    function app_tabs(int $tabs, int $spaces = 4): string
    {
        return str_repeat(app_tab($spaces), $tabs);
    }
}

if ( ! function_exists('app_nl')) {
    function app_nl(int $count = 1): string
    {
        return str_repeat(PHP_EOL, $count);
    }
}

if ( ! function_exists('app_nls')) {
    function app_nls(int $count, int $nls = 1): string
    {
        return str_repeat(app_nl($nls), $count);
    }
}

if ( ! function_exists('app_nl_tab')) {
    function app_nl_tab(int $lns = 1, int $tabs = 1): string
    {
        return app_nls($lns).app_tabs($tabs);
    }
}
