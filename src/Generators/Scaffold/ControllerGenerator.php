...
class ControllerGenerator extends BaseGenerator
{
    ...
    public function generate()
    {
        ...
        $this->config->commandComment(app_nl().'Controller created: ');
    ...
    protected function generateDataTable()
    {
        $templateData = view('laravel-generator::scaffold.table.datatable', [
            'columns' => implode(','.app_nl_tab(1, 3), $this->generateDataTableColumns()),
        ])->render();
        ...
        $this->config->commandComment(app_nl().'DataTable created: ');
    ...
    protected function generateLivewireTable()
    {
        $templateData = view('laravel-generator::scaffold.table.livewire', [
            'columns' => implode(','.app_nl_tab(1, 3), $this->generateLivewireTableColumns()),
        ])->render();
        ...
        $this->config->commandComment(app_nl().'LivewireTable created: ');
        $this->config->commandInfo($fileName);
    }
    ...
    protected function generateLivewireTableColumns(): array
    {
        ...
        foreach ($this->config->fields as $field) {
            ...
            $fieldTemplate = 'Column::make("'.$field->getTitle().'", "'.$field->name.'")'.app_nl();
            $fieldTemplate .= app_tabs(4).'->sortable()';

            if ($field->isSearchable) {
                $fieldTemplate .= app_nl().app_tabs(4).'->searchable()';
            }
